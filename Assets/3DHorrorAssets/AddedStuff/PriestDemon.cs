...
{
    public class PriestDemon : MonoBehaviour
    {
        ...
        public float ChaseSpeed = 4.5f; // ADDED
        public float PatrolSpeed = 3f; // ADDED
        public SphereCollider collider; // ADDED
        public bool inRange = false; // ADDED
        /*
            ...
            if (Time.time >= LastBehoviourTime + BehoviourPeriod)
            {
                ...
                if (inRange) // TEST
                {
                    ...
                    if (Distance <= ChaseRange)
                    {
                        ...
                        agent.speed = ChaseSpeed; // ADDED
                    }
                    else
                    {
                        target = null;
                        agent.speed = PatrolSpeed; // ADDED
                    }
                }
                else
                {
                ...
                /*
                Distance = Vector3.Distance(transform.position, HeroPlayerScript.Instance.transform.position); // UNCOMMENT
                if (Distance <= ChaseRange)
                {
                    target = HeroPlayerScript.Instance.transform;
                    agent.speed = ChaseSpeed; // ADDED
